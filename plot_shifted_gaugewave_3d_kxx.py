...
Time-stamp: <2014-02-28 14:10:18 (jonah)>
...
Example call:
python2 plot_shifted_gaugewave_kxx.py 0 *curv.x.asc
"""
...
def shifted_gaugewave_kxx(x,t):
    ...
    z = 0
    D = pg.D/np.sqrt(3.0)
    angle = 2*np.pi*(3*t + np.sqrt(3.0)*(-x+y+z))/(3.0*D)
    numerator = -pg.A*np.pi*np.cos(angle)
    denominator = 3.0*D*np.sqrt(1 - pg.A*np.sin(angle))
...
def plot_kxx(positions_list,kxx_list,filename_list,time):
    ...
    pg.plot_Txx(positions_list,kxx_list,filename_list,
                time,ylabel,shifted_gaugewave_kxx)
...
def plot_h4_errors(positions_list,kxx_list,h_list, filename_list,time):
    ...
    """
    pg.plot_errors(shifted_gaugewave_kxx,positions_list,kxx_list,h_list,
